...
std::vector<torch::Tensor> ba_with_prior(
    torch::Tensor poses,
    torch::Tensor disps,
    torch::Tensor intrinsics,
    torch::Tensor disps_sens,
  ...
  CHECK_INPUT(ii);
  CHECK_INPUT(jj);
  CHECK_INPUT(pose_prior_H);
  CHECK_INPUT(pose_prior_v);
  CHECK_INPUT(pose_prior_ii);
  CHECK_INPUT(pose_prior_jj);
...
std::vector<torch::Tensor> ba_with_full_prior(
    torch::Tensor poses,
    torch::Tensor velocities,
    torch::Tensor bias_acc,
    torch::Tensor bias_gyro,
  ...
  CHECK_INPUT(weights);
  CHECK_INPUT(poses);
  CHECK_INPUT(velocities);
  CHECK_INPUT(bias_acc);
  CHECK_INPUT(bias_gyro);
  CHECK_INPUT(disps);
  CHECK_INPUT(intrinsics);
  CHECK_INPUT(disps_sens);
  CHECK_INPUT(ii);
  CHECK_INPUT(jj);
  CHECK_INPUT(state_prior_H);
  CHECK_INPUT(state_prior_v);
  CHECK_INPUT(state_prior_ii);
  CHECK_INPUT(state_prior_jj);

  return ba_cuda_with_full_prior(
      poses, velocities, bias_acc, bias_gyro, disps, intrinsics, disps_sens,
      targets, weights, eta, ii, jj, t0, t1, iterations, lm, ep, motion_only,
      state_prior_H, state_prior_v, state_prior_ii, state_prior_jj);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("ba_with_prior", &ba_with_prior, "bundle adjustment with external pose prior");
  m.def("ba_with_full_prior", &ba_with_full_prior, "bundle adjustment with external full inertial prior");
