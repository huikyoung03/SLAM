...
__global__ void motion_retr_kernel(
    torch::PackedTensorAccessor32<float,2,torch::RestrictPtrTraits> velocities,
    torch::PackedTensorAccessor32<float,2,torch::RestrictPtrTraits> bias_acc,
    torch::PackedTensorAccessor32<float,2,torch::RestrictPtrTraits> bias_gyro,
    const torch::PackedTensorAccessor32<float,2,torch::RestrictPtrTraits> dx,
    const int t0, const int t1)
{
  for (int k=t0+threadIdx.x; k<t1; k+=blockDim.x) {
    ...
    for (int n=0; n<3; n++) {
      velocities[k][n] += dx[ix][6 + n];
      bias_acc[k][n] += dx[ix][9 + n];
      bias_gyro[k][n] += dx[ix][12 + n];
    }
  }
}
...
class SparseBlock {
  public:
    ...
    void add_lhs(torch::Tensor As, torch::Tensor ii, torch::Tensor jj) {
      ...
      Eigen::SparseMatrix<double> B(N*M, N*M);
      B.setFromTriplets(tripletList.begin(), tripletList.end());
      A += B;
    }

    void update_lhs_head(torch::Tensor As, torch::Tensor ii, torch::Tensor jj, const int K) {
    ...
    void add_lhs_head(torch::Tensor As, torch::Tensor ii, torch::Tensor jj, const int K) {
    ...
    void add_rhs(torch::Tensor bs, torch::Tensor ii) {
      update_rhs(bs, ii);
    }

    void update_rhs_head(torch::Tensor bs, torch::Tensor ii, const int K) {
      auto bs_cpu = bs.to(torch::kCPU).to(torch::kFloat64);
      auto ii_cpu = ii.to(torch::kCPU).to(torch::kInt64);
    ...
    void add_rhs_head(torch::Tensor bs, torch::Tensor ii, const int K) {
      update_rhs_head(bs, ii, K);
    }
...
SparseBlock schur_block_state(torch::Tensor E,
    torch::Tensor Q,
    torch::Tensor w,
    torch::Tensor ii,
    torch::Tensor jj,
    ...
    const int state_dim)
{
  ...
  auto ii_cpu = ii.to(torch::kCPU);
  auto jj_cpu = jj.to(torch::kCPU);
  auto kk_cpu = kk.to(torch::kCPU);

  auto ii_accessor = ii_cpu.accessor<LongType,1>();
  auto jj_accessor = jj_cpu.accessor<LongType,1>();
  auto kk_accessor = kk_cpu.accessor<LongType,1>();
  ...
  SparseBlock A(P, state_dim);
  A.update_lhs_head(S, ii2_cpu, jj2_cpu, 6);
  A.update_rhs_head(v, jj_cpu - t0, 6);

  return A;
}


std::vector<torch::Tensor> ba_cuda_with_prior(
    torch::Tensor poses,
    torch::Tensor disps,
    ...
    torch::Tensor pose_prior_H,
    torch::Tensor pose_prior_v,
    torch::Tensor pose_prior_ii,
    torch::Tensor pose_prior_jj)
{
  ...
  const int wd = disps.size(2);
  const int P = t1 - t0;
  const bool has_pose_prior = (
    pose_prior_H.numel() > 0 &&
    pose_prior_v.numel() > 0 &&
    pose_prior_ii.numel() > 0 &&
    pose_prior_jj.numel() > 0
  );
  ...
  for (int itr=0; itr<iterations; itr++) {
    ...
    if (has_pose_prior) {
      A.add_lhs(pose_prior_H, pose_prior_ii, pose_prior_jj);
      torch::Tensor pose_prior_rhs_ii = torch::arange(
          0, P, torch::TensorOptions().dtype(torch::kInt64));
      A.add_rhs(pose_prior_v, pose_prior_rhs_ii);
    }
...
std::vector<torch::Tensor> ba_cuda(
    ...
    const bool motion_only)
{
  torch::Tensor empty_H = torch::empty({0, 6, 6}, poses.options());
  torch::Tensor empty_v = torch::empty({0, 6}, poses.options());
  torch::Tensor empty_i = torch::empty({0}, ii.options());

  return ba_cuda_with_prior(
      poses, disps, intrinsics, disps_sens, targets, weights, eta, ii, jj,
      t0, t1, iterations, lm, ep, motion_only,
      empty_H, empty_v, empty_i, empty_i);
}


std::vector<torch::Tensor> ba_cuda_with_full_prior(
    torch::Tensor poses,
    torch::Tensor velocities,
    torch::Tensor bias_acc,
    torch::Tensor bias_gyro,
    torch::Tensor disps,
    torch::Tensor intrinsics,
    torch::Tensor disps_sens,
    torch::Tensor targets,
    ...
    const float ep,
    const bool motion_only,
    torch::Tensor state_prior_H,
    torch::Tensor state_prior_v,
    torch::Tensor state_prior_ii,
    torch::Tensor state_prior_jj)
{
  auto opts = poses.options();
  const int num = ii.size(0);
  const int ht = disps.size(1);
  ...
  const bool has_state_prior = (
    state_prior_H.numel() > 0 &&
    state_prior_v.numel() > 0 &&
    state_prior_ii.numel() > 0 &&
    state_prior_jj.numel() > 0
  );
  ...
  for (int itr=0; itr<iterations; itr++) {
    ...
    if (has_state_prior) {
      A.add_lhs(state_prior_H, state_prior_ii, state_prior_jj);
      torch::Tensor state_prior_rhs_ii = torch::arange(
          0, P, torch::TensorOptions().dtype(torch::kInt64));
      A.add_rhs(state_prior_v, state_prior_rhs_ii);
    }
    ...
    if (motion_only) {
      ...
      motion_retr_kernel<<<1, THREADS>>>(
        velocities.packed_accessor32<float,2,torch::RestrictPtrTraits>(),
        bias_acc.packed_accessor32<float,2,torch::RestrictPtrTraits>(),
        bias_gyro.packed_accessor32<float,2,torch::RestrictPtrTraits>(),
        dx.packed_accessor32<float,2,torch::RestrictPtrTraits>(), t0, t1);
    }
      ...
      torch::Tensor Ei = accum_cuda(Eii.view({num, 6*ht*wd}), ii, ts).view({P, 6, ht*wd});
      torch::Tensor E = torch::cat({Ei, Eij}, 0);

      SparseBlock S = schur_block_state(
          E, Q, w, ii_exp, jj_exp, kk_exp, t0, t1, state_dim);
      dx = (A - S).solve(lm, ep);
